...
| | |
| --- | --- |
| `agent::avatar(pose)` | The canvas fills its layout bounds and centres the design space inside. |
| colour | The caller's; left unset it follows `theme.accent`, and the eye ink comes from whichever end of the theme reads as a hole in that body. |
| `agent::mascot(face, t)` | Takes the `Face` rather than a `Pose`, since it samples the silhouette per cell instead of tracing an outline — which is why it cannot draw a blend of two faces and the spline painter can. Eyes are punched through, so a row dims whole. |
