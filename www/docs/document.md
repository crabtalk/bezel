---
title: Document
description: A reader with an outline, an editor and a source view — the screen bezel-markdown exists for, and the round trip you can see.
---

Nothing on this page is library code. The reader is an outline, a scroll area and a segmented toggle; the calls into the library are `bezel_markdown::render` and `bezel_markdown::Editor`, the latter behind the crate's `editor` feature.
