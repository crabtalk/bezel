...
`accent` is neutral by default. A library that ships a hue puts that hue in every app that installs it, so the default is the accent's lightness with the chroma at zero — this is the token to brand:
...
Alongside the fields are four context-free helpers — `ink(alpha)` for fills, `hairline(alpha)` for borders, `wash(alpha)` for state, `scrim(alpha)` for modal backdrops. They resolve against the installed appearance without a `cx`, so element builders deep in a tree can reach them. Every alpha is quoted in dark-mode terms and the light value is derived, which is why fills keep their number across appearances while hairlines scale up: a 1px edge needs more ink on a bright field, a plate needs less.
