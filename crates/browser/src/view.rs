...
impl WebView {
    pub fn new(url: impl Into<String>, window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let subscriptions = [
            cx.on_focus(&focus, window, |this: &mut Self, _window, _| {
                #[cfg(target_os = "windows")]
                this.page.keys.watch(_window);
                this.page.take_keys()
            }),
...
impl Render for WebView {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        #[cfg(target_os = "windows")]
        if self.page.focus.is_focused(_window) {
            self.page.keys.watch(_window);
        }
        div()
            .size_full()
...
impl Page {
    fn new(
        ...
        Self {
            ...
            #[cfg(target_os = "windows")]
            keys: Rc::new(accelerator::Keys::new(_cx)),
        }
    }
}
...
impl Page {
    ...
    fn attach(&self, view: &wry::WebView) {
        ...
        accelerator::attach(&controller, self.keys.clone(), self.reports.clone());
...
mod accelerator {
    ...
    fn named(vkey: VIRTUAL_KEY) -> Option<&'static str> {
        ...
        Some(match vkey {
            ...
            VK_DELETE => "delete",
            VK_APPS => "menu",
            vkey if (VK_F1.0..=VK_F24.0).contains(&vkey.0) => {
                FUNCTION[usize::from(vkey.0 - VK_F1.0)]
            }
            _ => return None,
        })
    }
